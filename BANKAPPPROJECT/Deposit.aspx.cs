...
namespace BANKAPPPROJECT
{
    public partial class Deposit : Page
    {
        ...
        protected void btnDeposit_Click(object sender, EventArgs e)
        {
            ...
            if (amt < 100)
            { Show("Minimum deposit is PHP 100.00.", false); return; }
            if (amt > 2000)
            { Show("Maximum deposit per transaction is PHP 2,000.00.", false); return; }
            if (amt % 100 != 0)
            ...
            using (var db = new SqlConnection(cs))
            {
                ...
                if (cur + amt > 10000)
                {
                    Show("Total balance cannot exceed PHP 10,000.00. Current: PHP " + cur.ToString("N2"), false);
                    LoadBalance(); return;
                }
                ...
                using (var tx = db.BeginTransaction())
                {
                    ...
                    {
                        cmd.Transaction = tx;
                        cmd.CommandText = "INSERT INTO Transactions(AccountNo,TxnType,Amount,Balance) VALUES(@a,'Deposit',@am,@b)";
                        cmd.Parameters.AddWithValue("@a", acct);
                ...
                Show("Deposited PHP " + amt.ToString("N2") + ". New balance: PHP " + newBal.ToString("N2"), true);
                txtAmount.Text = "";
            }
            LoadBalance();
        }
