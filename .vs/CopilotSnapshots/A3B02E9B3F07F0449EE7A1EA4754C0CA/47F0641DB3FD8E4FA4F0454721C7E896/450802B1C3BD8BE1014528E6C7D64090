<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Register.aspx.cs" Inherits="BANKAPPPROJECT.Register" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
<head runat="server">
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Register — Aurelia Private</title>
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,300;9..144,400;9..144,500&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />
    <style>
        :root {
            --bg: #0c1713;
            --surface: #121e1a;
            --surface-2: #1a2722;
            --text: #f1ead8;
            --muted: #8a9590;
            --primary: #27b487;
            --primary-700: #1c8a67;
            --champagne: #d4b06a;
            --champagne-soft: #e4c896;
            --ink: #07100d;
            --border: rgba(241,234,216,0.08);
            --border-strong: rgba(241,234,216,0.16);
            --danger: #e56b7a;
            --success: #27b487;
            --radius: 18px;
            --radius-sm: 10px;
            --shadow-sm: 0 2px 8px rgba(0,0,0,0.35);
            --shadow: 0 30px 60px -20px rgba(0,0,0,0.7), 0 8px 20px -8px rgba(0,0,0,0.5);
            --shadow-glow: 0 0 60px -10px rgba(39,180,135,0.35);
            --gradient-noir: linear-gradient(135deg, #07100d 0%, #122019 100%);
            --gradient-emerald: linear-gradient(135deg, #0f2a22 0%, #1d4a3b 100%);
            --gradient-gold: linear-gradient(135deg, #e4c896 0%, #b08a3f 100%);
        }

        * { box-sizing: border-box; margin: 0; padding: 0; }

        html, body {
            min-height: 100vh;
            background: var(--bg);
            color: var(--text);
            font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
            padding: 24px;
            -webkit-font-smoothing: antialiased;
            line-height: 1.55;
            font-feature-settings: "ss01", "cv11";
        }

        /* subtle grain */
        body::before {
            content: "";
            position: fixed; inset: 0;
            background-image: radial-gradient(rgba(241,234,216,0.05) 1px, transparent 1px);
            background-size: 3px 3px;
            opacity: 0.35; pointer-events: none; mix-blend-mode: overlay;
            z-index: 1;
        }

        .register-wrap {
            width: 100%;
            max-width: 460px;
            position: relative;
            z-index: 2;
        }

        .brand {
            display: flex;
            align-items: center;
            justify-content: center;
            gap: 14px;
            margin-bottom: 40px;
            color: var(--text);
            font-family: 'Fraunces', serif;
            font-size: 22px;
            font-weight: 400;
            letter-spacing: 0.02em;
        }

        .brand-mark {
            width: 38px; height: 38px;
            border-radius: 999px;
            background: var(--gradient-gold);
            display: grid; place-items: center;
            color: var(--ink);
            font-family: 'Fraunces', serif;
            font-weight: 500;
            font-size: 18px;
            box-shadow: 0 4px 14px rgba(212,176,106,0.25);
        }

        .brand span:last-child {
            display: flex;
            flex-direction: column;
            line-height: 1;
        }

        .brand small {
            font-family: 'Inter', sans-serif;
            font-size: 9px;
            text-transform: uppercase;
            letter-spacing: 0.3em;
            color: var(--muted);
            margin-top: 4px;
            font-weight: 500;
        }

        .card {
            background: var(--surface);
            border: 1px solid var(--border);
            border-radius: var(--radius);
            padding: 40px;
            box-shadow: var(--shadow-sm);
        }

        .card h2 {
            font-family: 'Fraunces', serif;
            font-size: 32px;
            font-weight: 300;
            color: var(--text);
            margin-bottom: 8px;
            letter-spacing: -0.015em;
            line-height: 1.1;
        }

        .card .subtitle {
            font-size: 13px;
            color: var(--muted);
            margin-bottom: 28px;
            letter-spacing: 0.02em;
        }

        label {
            display: block;
            margin: 18px 0 8px;
            font-size: 10px;
            font-weight: 500;
            color: var(--muted);
            text-transform: uppercase;
            letter-spacing: 0.25em;
        }

        input {
            width: 100%;
            padding: 14px 18px;
            border: 1px solid var(--border-strong);
            border-radius: var(--radius-sm);
            font-size: 14px;
            font-family: 'Inter', sans-serif;
            color: var(--text);
            background: var(--bg);
            outline: none;
            transition: border-color .2s ease, box-shadow .2s ease, background .2s ease;
        }

        input::placeholder { color: var(--muted); }

        input:focus {
            border-color: var(--primary);
            box-shadow: 0 0 0 3px rgba(39,180,135,0.15);
            background: var(--surface-2);
        }

        .btn {
            width: 100%;
            padding: 13px 28px;
            background: var(--gradient-emerald);
            color: var(--text) !important;
            border: 1px solid var(--border-strong);
            border-radius: var(--radius-sm);
            font-size: 12px;
            font-weight: 500;
            letter-spacing: 0.12em;
            text-transform: uppercase;
            font-family: 'Inter', sans-serif;
            cursor: pointer;
            margin-top: 28px;
            transition: all .25s cubic-bezier(0.22, 1, 0.36, 1);
        }

        .btn:hover {
            border-color: var(--primary);
            box-shadow: var(--shadow-glow);
            transform: translateY(-1px);
        }

        .msg {
            padding: 14px 18px;
            border-radius: var(--radius-sm);
            margin-top: 16px;
            font-size: 13px;
            font-weight: 500;
            border: 1px solid;
        }

        .err {
            background: rgba(229,107,122,0.08);
            color: var(--danger);
            border-color: rgba(229,107,122,0.3);
        }

        .ok {
            background: rgba(39,180,135,0.08);
            color: var(--primary);
            border-color: rgba(39,180,135,0.3);
        }

        .footer-link {
            text-align: center;
            margin-top: 24px;
            font-size: 13px;
            color: var(--muted);
            letter-spacing: 0.02em;
        }

        .footer-link a {
            color: var(--champagne);
            font-weight: 500;
            text-decoration: none;
            transition: color .2s ease;
        }

        .footer-link a:hover {
            color: var(--champagne-soft);
        }

        .divider {
            border: none;
            border-top: 1px solid var(--border);
            margin: 28px 0 0;
        }

        .two-col {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 12px;
        }

        @media (max-width: 480px) {
            .two-col { grid-template-columns: 1fr; }
            .card { padding: 28px; }
            .card h2 { font-size: 26px; }
        }
    </style>
</head>
<body>
<form id="form1" runat="server">
<div class="register-wrap">
    <div class="brand">
        <span class="brand-mark">A</span>
        <span>
            Aurelia
            <small>Private</small>
        </span>
    </div>
    <div class="card">
        <h2>Create an account</h2>
        <p class="subtitle">Fill in the details below to get started.</p>

        <div class="two-col">
            <div>
                <label>Username</label>
                <asp:TextBox ID="txtUsername" runat="server" />
            </div>
            <div>
                <label>Full Name</label>
                <asp:TextBox ID="txtFullName" runat="server" />
            </div>
        </div>

        <label>Email</label>
        <asp:TextBox ID="txtEmail" runat="server" TextMode="Email" />

        <hr class="divider" style="margin-top:20px;" />

        <div class="two-col">
            <div>
                <label>Password</label>
                <asp:TextBox ID="txtPassword" runat="server" TextMode="Password" />
            </div>
            <div>
                <label>Confirm Password</label>
                <asp:TextBox ID="txtConfirm" runat="server" TextMode="Password" />
            </div>
        </div>

        <asp:Button ID="btnRegister" runat="server" Text="Create Account" CssClass="btn" OnClick="btnRegister_Click" />
        <asp:Label ID="lblMsg" runat="server" />

        <hr class="divider" />
        <div class="footer-link">
            Already have an account? <a href="Login.aspx">Sign in</a>
        </div>
    </div>
</div>
</form>
</body>
</html>